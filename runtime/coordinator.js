...
const autoDiscardableByTab = new Map();
...
function sanitizeEntry(tabId, previous = {}, patch = {}) {
  const state = Object.values(RUNTIME_STATES).includes(patch.state)
    ? patch.state
    : (previous.state || RUNTIME_STATES.IDLE);
  return {
    tabId,
...
async function syncAutoDiscardable(entry, now = Date.now()) {
  ...
  try {
    ...
    autoDiscardableByTab.set(entry.tabId, desired);
  } catch {
    autoDiscardableByTab.delete(entry.tabId);
  }
}
...
chrome.runtime.onConnect.addListener(port => {
  ...
  port.onMessage.addListener(message => {
    handlePortMessage(port, message).catch(error => {
      console.warn('[TabFlow Runtime] Port message failed:', error?.message || error);
    });
  });
...
chrome.tabs.onReplaced?.addListener((addedTabId, removedTabId) => {
  portsByTab.delete(removedTabId);
  autoDiscardableByTab.delete(removedTabId);
  removeTab(removedTabId).catch(() => {});
  autoDiscardableByTab.delete(addedTabId);
});
